...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        # self.win_size = 1
        # # Embedding
        # self.enc_embedding1 = DataEmbedding(configs.enc_in, configs.d_model, configs.embed, configs.freq,
        #                                    configs.dropout)
        ...
        # self.first1 = nn.Sequential(
        #     # nn.LayerNorm(configs.seq_len),
        #     # TwoStageAttentionLayernoseg(configs, self.seg_num, configs.factor, configs.d_model,
        #     #                             configs.n_heads, configs.d_ff, configs.dropout),
        #     DataEmbedding_wo_pos(configs.enc_in, configs.d_model, configs.embed, configs.freq,
        #                                                      configs.dropout),
        #     nn.GELU(),
        #     nn.Dropout(configs.dropout)
        # )
        self.first1 = DataEmbedding_wo_pos(configs.enc_in, configs.d_model, configs.embed, configs.freq,
                                                             configs.dropout)
        ...
        # self.proo = nn.Linear()
        # self.proj = nn.Linear(configs.seq_len, configs.enc_in, bias=True)
        # self.prov = nn.Linear(configs.dec_in, configs.d_model,bias=True)

        self.MLP1 = nn.Sequential()
        self.MLP1.add_module('Linear1',nn.Linear(configs.enc_in, 512))
        self.MLP1.add_module('GELU', nn.GELU())
        self.MLP1.add_module('Linear2', nn.Linear(512, configs.enc_in))

        self.linear = nn.Linear(configs.seq_len,configs.pred_len)
        # self.MLP2 = nn.Sequential()
        # self.MLP2.add_module('Linear1', nn.Linear(configs.seq_len, 2048))
        # self.MLP2.add_module('GELU', nn.GELU())
        # self.MLP2.add_module('Linear2', nn.Linear(2048, configs.d_model))
        ...
        self.Norm6 = nn.LayerNorm(configs.d_model)
        self.Norm7 = nn.LayerNorm(configs.d_model)
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec,
        ...
        x_enclow = self.updimlow(x_enclow).permute(0, 2, 1)
        x_enchigh = self.updimhigh(x_enchigh).permute(0, 2, 1)
        # x_enchigh = self.first1(x_enchigh,None)
        # x_enchigh = self.enc_embedding1(x_enchigh, None)
        x_encforcast = self.updimforcast(x_encforcast).permute(0, 2, 1)
        # x_encforcast = self.enc_embedding2(x_encforcast, None)
        enc_out = self.encoder(x_enchigh)
        dec_out = self.decoder(enc_out)
        dec_out = self.Norm1(dec_out + enc_out)
        dec_out2 = self.decoder(dec_out)
        dec_outq1 = self.Norm2(dec_out2)

        # enc_out = self.encoder(x_encforcast)
        # dec_out = self.decoder(enc_out)
        # dec_out = self.Norm3(dec_out + dec_out)
        # dec_out2 = self.decoder(dec_out)
        # dec_outq2 = self.Norm4(dec_out2)

        enc_out = self.encoder(x_enclow)
        dec_out = self.decoder(enc_out)
        dec_out = self.Norm5(dec_out + enc_out)
        dec_out2 = self.decoder(dec_out)
        dec_outq3 = self.Norm6(dec_out2)
        # dec_outq2 = self.MLP2(x_encforcast)
        dec_out = self.Norm7(dec_outq3+dec_outq1).permute(0, 2, 1)
        # dec_out = self.proj(dec_out)
        # linear_out = self.linear(self.MLP1(x_enclow).permute(0, 2, 1)).permute(0, 2, 1)
